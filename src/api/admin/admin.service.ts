...
export class AdminService
	...
	async getStats(): Promise<ISuccess> {
		// Parallel ravishda barcha count so'rovlarini bajaramiz
		const [totalStudents, totalTeachers, totalLessons, allPayments] = await Promise.all([
			this.studentRepo.count(),
			this.teacherRepo.count(),
			this.lessonRepo.count(),
			// 'amount' o'rniga 'totalLessonAmount' ni tanlaymiz
			this.paymentRepo.find({ select: ['totalLessonAmount'] }),
		]);

		// Umumiy tushumni hisoblash
		const totalRevenue = allPayments.reduce((sum, p) => sum + Number(p.totalLessonAmount || 0), 0);
