...
export class BaseService<CreateDto, UpdateDto, Entity> {
	...
	async findAll(options?: IFindOptions<Entity>): Promise<ISuccess> {
		const data = await this.repository.find({
			...
			where: [
				...
				{ ...options?.where, isDelete: IsNull() } // isDelete hali o'rnatilmaganlarni ham oladi
			],
		});
		return successRes(data);
	}

	async findDeleted(options?: IFindOptions<Entity>): Promise<ISuccess> {
		const data = (await this.repository.find({
			...options,
			where: { ...options?.where, isDelete: true }, // Faqat O'CHIRILGANLARNI oladi
		})) as Entity[];
		return successRes(data);
	}
	...
	async softDelete(
		...
		const updateData = await this.repository.update(id, user);
